...
echo '' > ${MAILBSD_CONFIG_FILE}
chown ${SYS_ROOT_USER}:${SYS_ROOT_GROUP} ${MAILBSD_CONFIG_FILE}
chmod 0400 ${MAILBSD_CONFIG_FILE}
...
Welcome to the MailBSD setup wizard.
We will ask you some simple questions required to setup a mail server.
If you encounter any issues, please report to our support forum:

https://forum.mailbsd.org
...
* Maildir path will be converted to lowercases.
* Please create this directory in lowcases.
* It cannot be /var/mail or /root.
* Mailboxes will be stored under : ${STORAGE_BASE_DIR}/${STORAGE_NODE}/
* Daily backup of SQL/LDAP DB will be stored under: /var/vmail/backup
...
echo "export STORAGE_BASE_DIR='${STORAGE_BASE_DIR}'" >> ${MAILBSD_CONFIG_FILE}
...
"No web server" "I don't need any web server" "off" \
...
if [ X"${web_server}" == X'NGINX' ]; then
    ...
    echo "export DISABLE_WEB_SERVER='YES'" >>${MAILBSD_CONFIG_FILE}
fi

echo "export WEB_SERVER='${WEB_SERVER}'" >>${MAILBSD_CONFIG_FILE}
...
while : ; do
    ${DIALOG} \
    --title "PREFERRED BACKEND TO STORE MAIL ACCOUNTS" \
    --radiolist "\

Please choose the one you're farmliar with for easy maintenance.
They all use the same webmail (SOGo) and admin panel (MailBSD-Admin).
No big feature differences between them.
...
echo "export BACKEND_ORIG='${BACKEND_ORIG}'" >> ${MAILBSD_CONFIG_FILE}
echo "export BACKEND='${BACKEND}'" >> ${MAILBSD_CONFIG_FILE}

# Read-only SQL user/role, used to query mail accounts in Postfix, Dovecot.
export VMAIL_DB_BIND_PASSWD="$(${RANDOM_STRING})"
echo "export VMAIL_DB_BIND_PASSWD='${VMAIL_DB_BIND_PASSWD}'" >> ${MAILBSD_CONFIG_FILE}

# For database management: vmail.
export VMAIL_DB_ADMIN_PASSWD="$(${RANDOM_STRING})"
echo "export VMAIL_DB_ADMIN_PASSWD='${VMAIL_DB_ADMIN_PASSWD}'" >> ${MAILBSD_CONFIG_FILE}

# For mlmmj mailing list manager
export MLMMJADMIN_API_AUTH_TOKEN="$(${RANDOM_STRING})"
echo "export MLMMJADMIN_API_AUTH_TOKEN='${MLMMJADMIN_API_AUTH_TOKEN}'" >> ${MAILBSD_CONFIG_FILE}

# For database management: vmail.
export NETDATA_DB_PASSWD="$(${RANDOM_STRING})"
echo "export NETDATA_DB_PASSWD='${NETDATA_DB_PASSWD}'" >> ${MAILBSD_CONFIG_FILE}
...
*************************************************************************
*                                                                       *
* Below file contains sensitive infomation (username/password),         *
* please do remember to *MOVE* it to a safe place after installation.   *
*                                                                       *
*  FILE : ${MAILBSD_CONFIG_FILE}                                        *
*                                                                       *
*************************************************************************
